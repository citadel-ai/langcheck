...
def test_eval_value():
    eval_value = is_float(['1', '2', '3'])
    assert eval_value.all()
    assert eval_value.any()

    eval_value = is_float(['1', 'a', 'b'])
    assert not eval_value.all()
    assert eval_value.any()

    eval_value = is_float(['a', 'b', 'c'])
    assert not eval_value.all()
    assert not eval_value.any()
