...
LOADER_MAP = {
    ...
    "load_auto_model_for_text_classification": load_auto_model_for_text_classification,
...
class ModelManager:
        ...
        for lang, lang_conf in conf.items():
            for metric_name, metric_conf in lang_conf.items():
                ...
                self.__set_model_for_metric(
                    language=lang, metric=metric_name, **metric_conf
                )
    ...
    @staticmethod
    def validate_config(
        config, language="all", metric="all", run_check_model_availability=False
    ) -> None:
        """
    ...
    def __set_model_for_metric(
        ...
        **kwargs,
        ...
        try:
            ...
            # Validate the change
            ModelManager.validate_config(
                self.config, language=language, metric=metric
            )
